...
### フック設計のアンチパターン — 検査境界の原則

フックの検査対象が曖昧だと、**コンテンツ（ユーザーが書いた文字列）をメタデータ（ツールの構造的情報）と誤認する**問題が発生する。

#### 原則: 検査対象を明確に限定する

フックは「何を検査しているか」を常に意識し、コンテンツとメタデータを区別すること。

| アンチパターン | 問題 | 対処 |
|--------------|------|------|
| コマンド文字列全体を grep する | `--body` 引数の中身（PR 本文、Mermaid 図等）が誤検出される | コマンド構造を解析し、引数の中身を除外してからパターンマッチする |
| transcript 全体を grep する | Read ツールで読んだソースコードや、対話中のテキストが誤検出される | 構造化ログ（後述「観測の設計」）を使い、transcript のスキャンを避ける |
| フックが自分自身の出力パターンを検査対象にする | フックのソースコード内のエラーメッセージテンプレートが「実際のイベント」として検出される（自己参照問題） | 検査対象を実際のイベントデータに厳密に限定する |

#### 実例

**コマンド引数の誤検出**: `commit-guard` が `gh pr merge` パターンで grep するが、`gh pr create --body "... gh pr merge の図解 ..."` のように body 内にパターンが含まれると誤ブロックする。

```bash
# NG: コマンド全体を grep
echo "$COMMAND" | grep -q "gh\s+pr\s+merge"

# OK: --body/--body-file 引数を除外してから grep
COMMAND_WITHOUT_BODY=$(echo "$COMMAND" | sed 's/--body\s\+"[^"]*"//g; s/--body-file\s\+\S\+//g')
echo "$COMMAND_WITHOUT_BODY" | grep -q "gh\s+pr\s+merge"
```

**transcript の誤検出**: friction-feedback フックが transcript を grep し、セッション中に Read ツールで読んだフックソースコード内の `[ブランチ戦略ガード]` ラベルを「実際の拒否」としてカウントする。実際の拒否 1 件に対し 13 件のノイズが報告された。

## 観測の設計（Observability）

強制レベル（L5-L2）は「ルールを守らせる」仕組みだが、「ルールがどう使われているか」を知る仕組みも必要になる。観測は強制レベルと**直交する概念**であり、全てのレベルのフックに適用できる。

### 検出と記録の分離

フックの deny/block イベントを集計する場合、**検出（各フック）と記録（集計フック）を分離する**設計を推奨する。

```
検出レイヤー（各 deny/block フック）
    │
    ├── deny/block 判定時に構造化ログに書き出す
    │   → .local-docs/friction-log.jsonl
    │
集計レイヤー（Stop フック等）
    │
    └── 構造化ログを読む（transcript をスキャンしない）
```

### なぜ transcript スキャンを避けるべきか

transcript には AI の出力、ユーザーの入力、ツールの結果が全て含まれる。この中から特定のイベントを grep で抽出しようとすると:

- Read ツールで読んだソースコード内のパターンが誤検出される
- ユーザーとの対話で言及されたフック名が誤検出される
- 検出精度が transcript のフォーマットに依存する（脆弱）

### 構造化ログの推奨フォーマット

```jsonl
{"timestamp":"2026-02-18T13:00:00Z","hook":"commit-guard","decision":"deny","label":"ブランチ戦略ガード"}
{"timestamp":"2026-02-18T13:05:00Z","hook":"worktree-guard","decision":"deny","label":"ワークツリーガード"}
```

最小限のフィールド:
- `timestamp`: ISO 8601 形式
- `hook`: フックスクリプト名
- `decision`: `deny` | `block` | `ask`
- `label`: 人間が読めるラベル

### フックからの書き出し例

```bash
# deny/block 判定時に friction-log に追記
FRICTION_LOG="${LOCAL_DOCS_DIR:-$PROJECT_ROOT/.local-docs}/friction-log.jsonl"
echo "{\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"hook\":\"commit-guard\",\"decision\":\"deny\",\"label\":\"ブランチ戦略ガード\"}" >> "$FRICTION_LOG"
```
