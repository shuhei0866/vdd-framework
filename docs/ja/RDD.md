...
Phase 4 のゴールは「develop にマージするところまでやり切ること」。実装エージェントに手順を逐一指示するのではなく、**ゴールを宣言し、Stop フック（L4）で完了条件を機械的に検証する**。

検証される完了条件の例:
- リリース仕様書がコミットされている
- レビューが実行されている（Phase 3 の成果）
- PR が統合ブランチにマージされている

エージェントは手段を自分で選ぶ。スキルを使っても、手動で操作しても、完了条件を満たせばゲートを通過できる。詳細は [philosophy.md](./philosophy.md) の「ゴール + 出口ゲート」を参照。
